...
public class SServerPlayer extends UDPClient
    implements Player {
  ...
  public void received(String msg) throws IOException {
    try {
      if (getLoglevel()>9) this.getLogger().println("<---'"+msg+"'");
      filter.run(msg, cmdBuf);
      cmdBuf.takeStep(controller, parser, this);
      while (commandFactory.hasNext()){
        String cmd = commandFactory.next();
        if (getLoglevel()>9) this.getLogger().println("--->'"+cmd+"'");
        send(cmd);
        pause(50);
      }
    } catch (Exception ex) {
      this.getLogger().write(ex.getMessage());
